...
const nameOf = (key) => t(`tools.${key}.name`)
const shortOf = (key) => t(`tools.${key}.short`)

const visibleTabs = computed(() => {
  const q = search.value.trim().toLowerCase()
  if (!q) return props.tabs
  return props.tabs.filter((tab) =>
    `${shortOf(tab.key)} ${nameOf(tab.key)}`.toLowerCase().includes(q),
  )
})
</script>
...
<template>
  <div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-violet-600 to-purple-500 px-5 py-5 mb-7 shadow-md">
      ...
      <p class="text-white/80 text-sm mt-1">{{ t('home.featuredSub') }}</p>
